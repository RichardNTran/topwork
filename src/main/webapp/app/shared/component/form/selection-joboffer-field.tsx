...
class SelectionJobOfferField extends Component<ISelectionProps> {
  componentDidMount() {
  ...
  getSelectedValue(field: ISelectItem, options: ISelectItem[]) {
    ...
    if (field && !_.isEmpty(field.label)) {
      selectedValue =
        field.label === 'none' ? _.findLast(options, { value: (field || {}).label }) : _.findLast(options, { label: (field || {}).label });
    }
...
const selectionJobOfferField = connect(
  mapStateToProps,
  mapDispatchToProps
)(SelectionJobOfferField);

export { selectionJobOfferField as SelectionJobOfferField };
